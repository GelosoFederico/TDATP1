...
def variante_Gale_Shapley(recitales_preferencias, bandas_preferencias, X, Y):
	...
	while len(recitales_libres)>0: #O(N)
		...
		for i in range(recitales_ult_propuesto[recital_act-1], M): #O(M)
			...
			elif banda_prefiere_recital_nuevo(banda_act, recital_act, recitales_por_banda, bandas_preferencias): #O(NY)
				recital_viejo = reemplazar_recital_viejo_por_nuevo(banda_act, recital_act, recitales_por_banda, bandas_por_recital, bandas_preferencias)#O(NY)
...
def banda_prefiere_recital_nuevo(banda_act, recital_act, recitales_por_banda, bandas_preferencias):  #O(NY)
	for x in range(len(bandas_preferencias[banda_act])-1, 0, -1): #O(N)
...
def reemplazar_recital_viejo_por_nuevo(banda_act, recital_act, recitales_por_banda, bandas_por_recital, bandas_preferencias):#O(NY)
	for x in range(len(bandas_preferencias[banda_act])-1, 0, -1): #O(N)
		if bandas_preferencias[banda_act][x] in recitales_por_banda[banda_act]: #O(Y)
